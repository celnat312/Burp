...
from credential_dontcommit.api_key import API_KEY, SEARCH_ENGINE_ID
...
def forgotpass(uname, pwd):
    global AccountTable
    AccountTable.forgotPassword(uname, pwd)
    AccountTable.commitChanges()
...
def bookmark(username, stri):
    ...
    items = stri.split('\n')
    url = items[2]
    print("bookmark data send to BE:", username, url)
...
# Recipe = Recipe()
def upload(dishName, urlLink, veg_bool, selected_cuisine, selected_ingredients, username1):
    global Recipe, idn
    likes = None
    uploaded = True
    file = None
    idn = generate(dishName)
    author = username1
    cuisine = selected_cuisine[0]
    ingred1 = selected_ingredients[0]

    Recipe.addRecipe(idn, dishName, uploaded, ingred1, cuisine, veg_bool, author, urlLink, IngredTag2=None,
                     IngredTag3=None)
    Recipe.commitChanges()
...
def pullupload(username):
    global Recipe
    n = Recipe.getAllPulledRecipe(username)
    return n
